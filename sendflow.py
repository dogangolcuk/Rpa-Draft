...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Send commands from a JSON file to a socket')
    parser.add_argument('file', metavar='file', type=str,
                        help='Path to the JSON file')
    parser.add_argument('--host', default='127.0.0.1',
                        help='Host IP for the socket (default: 127.0.0.1)')
    parser.add_argument('--port', type=int, default=12345,
                        help='Port for the socket (default: 12345)')

    args = parser.parse_args()

    send_commands_to_socket(args.file, args.host, args.port)
    print(f'Sent commands from {args.file} to AGENT {args.host}:{args.port}')
